...
const char* testUnkStr="/home/phoenix/oracle/corpus/src/test-unknown.txt";
const char* wordmapStr="/home/phoenix/oracle/corpus/src/word-map.txt";
const char* trainwordStr="/home/phoenix/oracle/corpus/src/train-word.txt";
...
//void test_text(Ngram* lm,vector<wstring> &testUnk,const wstring& unk,priority_queue<wordppl,vector<wordppl>,less<vector<wordppl>::value_type> >& res)
void* test_text(void* args)
{
	Ngram* lm = ((thread_args*)args)->lm;
	//vector<wstring> &testUnk = ((thread_args*)args)->testUnk;
	wstring &unk = ((thread_args*)args)->unk;
	priority_queue<wordppl,vector<wordppl>,less<vector<wordppl>::value_type> > &res = ((thread_args*)args)->res_heap;
	...
	for (vector<wstring>::iterator i=trainword.begin();i!=trainword.end();i++)
	{
		...
		lm->dout(cout);
		lm->pplFile(file, stats, escape);
		lm->dout(cerr);

		res.push(wordppl(*i,stats.getPPL()));

		cout << stats.getPPL() << endl;
	}
...
int main(int argc,char* argv[])
{
	...
	while (getline(wordmapinf,origin))
	{
		...
		args[count].lm=lm;
		args[count].origin=origin;
		args[count].unk=unk;
		args[count].res_heap=priority_queue<wordppl,vector<wordppl>,less<vector<wordppl>::value_type> >();

		pthread_create(threads+count,NULL,test_text,args+count);
		count++;
		...
		if (count >= MAX_THREAD)
		{
			for (int i=0;i<MAX_THREAD;i++)
			{
				...
				resouf << args[i].origin << ":  ";	
				for (int i=0;args[i].res_heap.size();i++)
				{
					resouf << args[i].res_heap.top().str << ' ' << args[i].res_heap.top().ppl << " | ";
					args[i].res_heap.pop();
				}
				resouf << endl << endl;
			}
