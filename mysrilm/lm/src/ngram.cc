...
vector<wstring> testUnk;
vector<wstring> context;

//string testUnkStr="/home/haizhao/oracle/SRILM/lm/bin/i686-m64/test-unknown.txt";
//string wordmapStr="/home/haizhao/oracle/SRILM/lm/bin/i686-m64/word-map.txt";
//string trainwordStr="/home/haizhao/oracle/SRILM/lm/bin/i686-m64/train-word.txt";
string testUnkStr="/home/phoenix/oracle/corpus/src/test-unknown.txt";
string wordmapStr="/home/phoenix/oracle/corpus/src/word-map-3.txt";
string trainwordStr="/home/phoenix/oracle/corpus/src/train-word.txt";
string resultStr="result.txt";
string tempOufStr = "/dev/shm/oracle/temp";
...
void gen_context(const vector<wstring> &text,const wstring unk,vector<wstring> &context)
{
	wstring str;
	for (int i=0;i<text.size();i++)
	{
		int pos = 0;
		wstring& line = const_cast<wstring&>(text[i]);
		while ((pos=line.find(unk,pos))!=string::npos)
		{
			int start = find_back(line,const_cast<wstring&>(unk),pos);
			int dest = find_forward(line,const_cast<wstring&>(unk),pos);

			context.push_back(line.substr(start,dest-start+1));
			pos++;
		}
	}
}
...
void replace_text(const wstring& src,const wstring& dest,const vector<wstring> &text,wofstream& ouf)
{
	wstring line;
	int len=src.length();
...
void test_text(Ngram* lm,const vector<wstring> &testUnk,const wstring& unk,priority_queue<wordppl,vector<wordppl>,less<vector<wordppl>::value_type> >& res)
{
	for (vector<wstring>::iterator i=trainword.begin();i!=trainword.end();i++)
	{
		wofstream tempouf(tempOufStr.c_str());
		
		replace_text(unk,*i,testUnk,tempouf);
		
		tempouf.close();

		File file(tempOufStr.c_str(), "r");
		TextStats stats;

		lm->dout(cout);
		lm->pplFile(file, stats, NULL);
		//lm->dout(cerr);

		res.push(wordppl(*i,stats.getPPL()));
	}
...
{
	if (argc>1)
		wordmapStr = string(argv[1]);
	if (argc>2)
		resultStr = string(argv[2]);
	if (argc>3)
		tempOufStr += string(argv[3]);


	locale::global(locale("en_US.UTF-8"));
	wifstream testUnkinf(testUnkStr.c_str());
	wifstream wordmapinf(wordmapStr.c_str());
	wifstream trainwordinf(trainwordStr.c_str());

	wofstream resouf(resultStr.c_str());
	...
	//Read test data with unknown tokens
	while (getline(testUnkinf,tmp))
		testUnk.push_back(tmp);
	
	//Read LM
	File file("/dev/shm/oracle/train-binary.lm", "r");
	Vocab *vocab;
	vocab = new Vocab;
	vocab->unkIsWord() = vocab->toLower() = false;

	Ngram *lm = new Ngram(*vocab, 3);
	...
	while (getline(wordmapinf,origin))
	{
		...
		context.clear();
		gen_context(testUnk,unk,context);

		test_text(lm,context,unk,res_heap);
